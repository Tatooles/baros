...
- The SDK applies the current Sentry scope and build to reports that may describe earlier activity. Baros moves the current release, dist, distribution channel, app/device/OS/locale/runtime context, and validated UI context into `diagnostic_delivery`. `received_at` is the Unix time when the report passes the final filter.
...
1. Build/run a Debug copy on a physical iPhone with build-setting overrides `SENTRY_ENABLED=YES` and `SENTRY_DSN=<existing project DSN from project.yml>`. Keep the Development environment and development bundle ID; do not replace the App Store installation or clear app data. Debug builds permit this explicit opt-in and label events `development`; Release builds still require Production. Ordinary Debug builds remain disabled. These are build settings, not launch environment variables. Start from a clean app launch so SDK subscriptions are installed.
...
6. Once those checks pass, explicitly set `enableAppHangTracking = false` in the main app's Sentry options, keeping `enableMetricKit` enabled. The pinned Cocoa 9.25.0 SDK has no `enableAppHangTrackingV2` option; V2 became the default in 9.0. Verify ordinary crash and sync reporting still work. Check that the relevant Sentry alerts include MetricKit warning events before relying on them. This cutover needs its own reviewed change if validation happens after this PR merges.

## Alert coverage

[Baros MetricKit diagnostics](https://kevin-tatooles.sentry.io/monitors/alerts/6044365/) covers `baros-ios` in `production`. It matches `diagnostic_timestamp_basis=payload_interval_start`, which the final scrubber adds to all three supported MetricKit mechanisms. There is no severity or priority filter, so warning events qualify. New, reappearing, and regressed issues notify Kevin's Sentry account, throttled to once per issue per 60 minutes. Development validation events remain searchable but do not notify through this rule. Configuration was verified on 2026-09-21; actual notification delivery remains a separate validation step.
