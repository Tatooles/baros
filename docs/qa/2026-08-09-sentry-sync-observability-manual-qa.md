...
6. Create an alert named `Baros durable sync failure` that emails the maintainer for events with `component=sync` and `outcome=failure`. Do not include `outcome=recovered`.
7. If Sentry cannot notify for every matching event, record the actual grouping or throttling behavior in issue #50 and use the closest event-frequency alert. Do not add an ingestion filter that discards stored events.

## Archive and symbols

1. Provide `SENTRY_AUTH_TOKEN` as an archive/CI secret. Do not add it to the repository, app bundle, or event data.
2. Archive the Release configuration. The archive must fail if `sentry-cli` or `SENTRY_AUTH_TOKEN` is unavailable.
